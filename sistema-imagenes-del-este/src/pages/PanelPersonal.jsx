...
{modalAbierto && pacienteActivo && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden animate-fadeIn border border-gray-100">
      
      {/* HEADER DEL MODAL */}
      <div className="bg-[#7A2328] text-white p-5 flex justify-between items-center">
        <div>
          <h3 className="font-bold text-lg leading-tight">{pacienteActivo.nombre_completo}</h3>
          <p className="text-xs text-red-100 mt-0.5">
            C.I: <span className="font-semibold">{pacienteActivo.cedula}</span> • 
            Correo: <span className="font-semibold">{pacienteActivo.correo || 'No registrado'}</span>
          </p>
        </div>
        <button
          onClick={() => setModalAbierto(false)}
          className="text-white/80 hover:text-white bg-white/10 hover:bg-white/20 w-8 h-8 rounded-full flex items-center justify-center transition text-sm font-bold"
        >
          ✕
        </button>
      </div>
      ...
      <div className="p-6 max-h-[60vh] overflow-y-auto">
        <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4">
          Estudios e Imágenes Cargadas
        </h4>

        {cargandoEstudios ? (
          <div className="py-8 text-center text-gray-500 flex flex-col items-center gap-2">
            <div className="w-6 h-6 border-2 border-[#7A2328] border-t-transparent rounded-full animate-spin"></div>
            <p className="text-xs font-medium">Buscando archivos en el sistema...</p>
          </div>
        ) : estudiosPaciente.length === 0 ? (
          <div className="py-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
            <p className="text-gray-500 text-sm font-medium">Este paciente aún no tiene estudios subidos.</p>
            <p className="text-xs text-gray-400 mt-1">Usa la opción de "Subir Estudio" en el menú para adjuntar una imagen.</p>
          </div>
        ) : (
            ...
            {estudiosPaciente.map((estudio) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#7A2328]/10 text-[#7A2328] rounded-lg flex items-center justify-center font-bold text-lg">
                    📄
                  </div>
                  <div>
                    <h5 className="font-bold text-gray-800 text-sm">{estudio.titulo}</h5>
                    <p className="text-xs text-gray-500">
                      {estudio.tipo_examen} • {new Date(estudio.fecha_estudio).toLocaleDateString()}
                    </p>
                  </div>
                </div>

                <a
                  href={`/api/descargar/${estudio.id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-[#7A2328] hover:bg-[#611B1E] text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1 shadow-sm"
                >
                  ⬇️ Descargar
                </a>
              </div>
