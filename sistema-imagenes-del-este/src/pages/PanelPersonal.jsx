...
export default function PanelPersonal() {
  ...
  const handleActualizarPaciente = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:3001/api/pacientes/${pacienteAEditar.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleVerEstudios = async (paciente) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/pacientes/${paciente.id}/estudios`);
      const data = await res.json();
  ...
  const handleGuardarEstudio = async (e) => {
    ...
    const res = await fetch('http://localhost:3001/api/estudios', {
      method: 'POST',
      body: formData
    });
